...
`ProductImage` is compose-only (`autoRegisterModels: true`, no cache). `Profile` is a Nest provider with `cache` so nested compose `getMany` is cached under `starter:repo:profile:q:…`.
